...
router.post("/", async (req, res) => {
  ...
  selected_defects.forEach(async(element) => {
    if (element.Segement === "Surface-RH-139") {
       let delete_defect = await Surface_RH_139_Schema.deleteOne({_id : element._id})
    ...
    if (element.Segement === "Bluetooth-139") {
        let delete_defect = await Bluetooth_139_Schema.deleteOne({_id : element._id})
        console.log(delete_defect)
     ...
     if (element.Segement === "Electrical-1-140") {
        let delete_defect = await Electrical_1_140_Schema.deleteOne({_id : element._id})
        console.log(delete_defect)
     ...
     if (element.Segement === "Surface-LH-140") {
        let delete_defect = await Surface_LH_140_Schema.deleteOne({_id : element._id})
        console.log(delete_defect)
     ...
     if (element.Segement === "Rear-Int-140") {
        let delete_defect = await Rear_Int_140_Schema.deleteOne({_id : element._id})
        console.log(delete_defect)
     ...
     if (element.Segement === "Rear-EXT-141") {
        let delete_defect = await Rear_EXT_141_Schema.deleteOne({_id : element._id})
        console.log(delete_defect)
     ...
     if (element.Segement === "RH-Exterior-141") {
        let delete_defect = await RH_Exterior_141_Schema.deleteOne({_id : element._id})
        console.log(delete_defect)
        let update_segement = await  new RH_Exterior_141_Schema(req.body.newDefect)
        let result = await update_segement.save();
        // console.log(res.send(result))
     }
     if (element.Segement === "LH-Exterior-141") {
        let delete_defect = await LH_Exterior_141_Schema.deleteOne({_id : element._id})
        console.log(delete_defect)
        let update_segement = await  new LH_Exterior_141_Schema(req.body.newDefect)
        let result = await update_segement.save();
        // console.log(res.send(result))
     }
     if (element.Segement === "Electrical-2-142") {
        let delete_defect = await Electrical_2_142_Schema.deleteOne({_id : element._id})
        console.log(delete_defect)
        let update_segement = await  new Electrical_2_142_Schema(req.body.newDefect)
        let result = await update_segement.save();
        // console.log(res.send(result))
     }
     if (element.Segement === "Front EXT-142") {
        let delete_defect = await Front_EXT_142_Schema.deleteOne({_id : element._id})
        console.log(delete_defect)
     ...
     if (element.Segement === "Door-Closing-142") {
        let delete_defect = await Door_Closing_142_Schema.deleteOne({_id : element._id})
        console.log(delete_defect)
        let update_segement = await  new Door_Closing_142_Schema(req.body.newDefect)
        let result = await update_segement.save();
        // console.log(res.send(result))
     }
